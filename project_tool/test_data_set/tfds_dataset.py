...
def download_dataset():
    ...
    ## 获取 builder，但不自动 prepare
    #dataset_name = "fractal20220817_data"
    #builder = tfds.builder(dataset_name, data_dir=DOWNLOAD_DIR)
    ## 手动构建数据（下载 + 解压 + 构建），禁用 Beam 默认并发，防止 recursion 错误
    #builder.download_and_prepare(try_gcs=False)
    ## 加载 tf.data.Dataset
